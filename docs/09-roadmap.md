...
- **Delivery-partner app hardening**: background location updates while app is backgrounded (within PWA capability limits, evaluating native wrapper if needed), incentive/bonus configuration, delivery-partner support/help flow.
- **Admin verification workflows** — **Partially done.** Supermarket self-registration (`POST /api/auth/register-supermarket-owner`) plus an admin approve/reject queue are live — new stores start `PENDING` and stay invisible to customer discovery until an admin reviews them; see root `README.md` → Admin & supermarket onboarding for how to set up a local admin account for testing (no seeded credential ships with the app anymore - see V10 migration). **Not yet started**: document viewer with annotation, rejection-reason templates, re-submission flow, SLA tracking on verification turnaround, and delivery-partner verification — the last of these is deferred until the delivery-partner app/module exists.
