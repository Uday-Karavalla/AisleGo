...
public class AddressController {
    ...
    public AddressResponse update(@AuthenticationPrincipal AuthenticatedUser principal,
                                   ...
                                   @Valid @RequestBody CreateAddressRequest request) {
        return addressService.update(principal.userId(), addressId, request);
    }

    @DeleteMapping("/{addressId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void delete(@AuthenticationPrincipal AuthenticatedUser principal, @PathVariable Long addressId) {
